...
describe('Token Tests', () => {
  ...
  test('JoinToken (no indexes)', async () => {
    ...
    expect(winnerStream.getIndex('getWinner').complete).toEqual(false);
  ...
  test('JoinToken (this index)', async () => {
    expect.assertions(3);
    ...
    await handStream.buildIndex('key');
    const joinedStream = handStream.join(['winnerStream', 'key', 'getWinner', 'finish'], {
      namedStreams: { winnerStream },
      namedFunctions: {
        finish: function * (hand, trick) {
    ...
    expect(joinedStream.getIndex('key').complete).toEqual(true);
    expect(winnerStream.getIndex('getWinner').complete).toEqual(false);
  ...
  test('JoinToken (both indexes)', async () => {
    expect.assertions(3);
